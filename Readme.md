...
* Generating coeditor networks
* Calculating coeditor network properties (degree, path length, mincut)
* Calculating WikiProject performance and efficiency
...
rely on indexes to finish in a reasonable amount of time. The indexes are
not all generated by the jupyter notebooks, so if a query is taking too long
it may be necessary to look into whether an index will speed it up. There are
some scripts to create indexes in the `sql` directory. Also note
that indexes must be created after data has been inserted, otherwise inserts
will take too long.

## Analysis scripts
These scripts are written in R and use the output of the jupyter notebooks
to perform OLS regressions.

### bga_performance
These R scripts perform regressions on B-GA performance: the fraction of B or
higher articles that reach GA or FA. The stage_*_eff scripts include
as an independent variable the stage efficiency: the grade transitions / edit
considering only edits made during an article's time spent at the previous
grade level.

### complete_performance
These R scripts perform regressions on complete performance: the fraction of
articles in a project that reach GA or Featured status. This is the measure
used in the paper.

### featured_performance
These R scripts perform regressions on featured performance: the fraction of
GA or higher articles that reach FA.

### life_efficiency
These R scripts perform reressions on lifetime efficiency: the grades/edit
over the entire lifetiem of articles.

### stage_efficiency
These R scripts perform reressions on stage efficiency: the grades/edit
over the period preceding each grade transition.

### total_efficiency
These R scripts perform reressions on total efficiency: the total grade
transitions in a project divided by the number of edits to articles below
that grade.
