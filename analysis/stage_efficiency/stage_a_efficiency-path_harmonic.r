...
vars <- c("path_harmonic", "degree_mean", "stage_N_a", "stage_Delta_a", "connectivity", "mean_editors", "article_count", "contributor_count", "first_assessment", "mean_article_age", "revision_count")
...
m_eff = lm(log(a_efficiency) ~ log(path_harmonic) + log(mean_editors) + log(article_count)  + log(contributor_count) + log(revision_count) + first_assessment + mean_article_age, data=df)
