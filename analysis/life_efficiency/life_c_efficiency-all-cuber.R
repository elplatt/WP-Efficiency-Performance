...
df <- filter(df, stage_N_c > 0)
df$c_efficiency = df$Articles_c / df$stage_N_c

m4_in_scale = lm(scale(log(c_efficiency)) ~ scale(connectivity) + scale(path_mean^(1/3)) + scale(in_degree_skew^(1/3)) + scale(degree_mean^(1/3)) + scale(talk_fraction^(1/3)) + scale(similarity_mean^(1/3)) + scale(mean_editors^(1/3)) + scale(article_count^(1/3))  + scale(contributor_count^(1/3)) + scale(revision_count^(1/3)) + scale(first_assessment) + scale(mean_article_age), data=df)
summary(m4_in_scale)
write.csv(tidy(m4_in_scale), "clife_in_scale.csv")

m4_out_scale = lm(scale(log(c_efficiency)) ~ scale(connectivity) + scale(path_mean^(1/3)) + scale(in_degree_skew^(1/3)) + scale(degree_mean^(1/3)) + scale(talk_fraction^(1/3)) + scale(similarity_mean^(1/3)) + scale(mean_editors^(1/3)) + scale(article_count^(1/3))  + scale(contributor_count^(1/3)) + scale(revision_count^(1/3)) + scale(first_assessment) + scale(mean_article_age), data=df)
